...
df = pd.read_csv('doublet_res.tsv', sep=r"\s+")
#df.barcode = df.barcode + '-' + df.sample_id
#df = df.drop(['sample_id'], axis=1)

# rm doublets
barcodes_sel = df[df.doublet != "yes"].barcode.str.replace("^(.*)_(.*)$", "\\2-\\1", regex=True).to_list()
#barcodes_sel = df[df.doublet != "yes"].barcode.to_list()
...
sc.pp.highly_variable_genes(adata, flavor='seurat_v3', n_top_genes=3000) #, batch_key='sample_id')
...
# tcr genes
df = pd.read_csv("hgnc_gene_group_tcr.txt", sep="\t")
df_ex = df

tcr_ig_genes = df_ex['Approved symbol'].to_list()
tcr_ig_genes.extend(["Abecma", "Carvykti"])
